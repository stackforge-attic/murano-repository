...
from muranorepository.consts import UI, AGENT, HEAT, WORKFLOW, SCRIPTS, MANIFEST
...
class TestDirOperations(FlaskTestCase):
    ...
    def test_create_workflow_subfolder(self):
        response = self.client.put(self.url_with_path.format(WORKFLOW, 'test'))
        self.assert200(response)
    ...
    def test_create_heat_subfolder(self):
        response = self.client.put(self.url_with_path.format(HEAT, 'test_heat'))
        self.assert200(response)
    ...
    def test_create_root_subfolder(self):
        response = self.client.put(self.url_with_path.format(MANIFEST, 'test'))
