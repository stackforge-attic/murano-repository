...
def get_archive_data(client_type):
    if client_type not in CLIENTS_DICT.keys():
        abort(404)
    path_to_archive = api._get_archive(client_type,
...
def download_service_archive(service_name):
    ...
    api._check_service_name(service_name)
...
def get_data_type_locations(data_type):
    api._check_data_type(data_type)
    result_path = api._compose_path(data_type)
    return api._get_locations(data_type, result_path)
...
def upload_file(data_type):
    api._check_data_type(data_type)
    try:
        return api._save_file(request, data_type)
    except:
        abort(403)
...
def _get_locations_in_nested_path_or_get_file(data_type, path):
    api._check_data_type(data_type)
    result_path = api._compose_path(data_type, path)
...
def upload_file_in_nested_path(data_type, path):
    ...
    return api._save_file(request, data_type, path)
...
def get_files_for_service(service_name):
    ...
    for manifest in manifests:
        if manifest.full_service_name == service_name:
            data = api._get_manifest_files(manifest)
            break
    if not data:
        abort(404)
...
def upload_new_service():
    path_to_archive = api._save_archive(request)
...
def delete_service(service_name):
    ...
    for manifest in manifests:
        if manifest.full_service_name == service_name:
            ...
            files_for_deletion = api._get_manifest_files(manifest_for_deletion)
            manifests.remove(manifest_for_deletion)
            break
    if not manifest_for_deletion:
        abort(404)

    files_for_deletion = api._exclude_common_files(files_for_deletion, manifests)

    return api._perform_deletion(files_for_deletion, manifest_for_deletion)
