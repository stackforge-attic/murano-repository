...
def _exclude_common_files(files_for_deletion, manifests):
    for manifest in manifests:
        ...
        for data_type, files in files_for_deletion.iteritems():
            data_type_files = manifest_files.get(data_type)
            if data_type_files:
                common_files = list(set(manifest_files[data_type]) &
                                    set(files_for_deletion[data_type]))
                if common_files:
                    files_for_deletion[data_type] = list(
                        set(files_for_deletion[data_type]) - set(common_files))
